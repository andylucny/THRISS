...
session = ort.InferenceSession("dino_deits8-224-final.onnx", providers=['CUDAExecutionProvider'])
...
image_size = (224, 224)
blob = cv.dnn.blobFromImage(frame, 1.0/255, image_size, swapRB=True, crop=True)
...
for i, attention in enumerate(attentions):
    ...
    if i == 2: # the best attention map index for close objects
        mask = cv.resize(attention,(frame.shape[1],frame.shape[0]))
